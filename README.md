...
> - **Testing** — CI and protocol tests use a local mock upstream. On 2026-09-19,
>   a manual synthetic live smoke test against the TypeSafe API succeeded via
>   direct HTTP, the JavaScript SDK and the local MCP bridge. This live test is
>   not part of automated CI; the Python SDK was not live-tested, and host E2E
>   tests in Claude Code, Codex and Cursor remain outstanding.
...
[examples/mcp-configs/](examples/mcp-configs/). The `.local.*` files are the
current local-build configurations: `claude-code.local.mcp.json`,
`codex.local.config.toml` and `cursor.local.mcp.json`.
...
The future npm configurations are `claude-code.npm.mcp.json`,
`codex.npm.config.toml` and `cursor.npm.mcp.json`; they are not usable until the
